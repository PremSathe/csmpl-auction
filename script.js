...
const themeMusic = new Audio("sounds/ipl3.mp3");
const soldMusic = new Audio("sounds/ipl2.mp3");
const bidTick = new Audio("sounds/bid_tick.mp3"); // Optional tick sound
...
function updateUI() {
  ...
  teams.forEach((t, i) => {
    ...
    // 3. Optimized Assignment Modal (Check affordability)
    const canAfford = t.budget >= currentBid;
    const isFull = t.squad.length >= 7;

    modal.innerHTML += `
      <button 
        onclick="${canAfford && !isFull ? `finalizeSale(${t.id})` : ""}" 
...
function loadPlayer() {
  ...
  if (p) {
    ...
    updateUI(); // Refresh modal team list based on current player bid
  } else alert("ID not found");
}
...
function raiseBid() {
  ...
  currentBid = Math.min(currentBid + inc, 50000);
  document.getElementById("bidDisplay").innerText = currentBid.toLocaleString();
  updateUI(); // Dynamic update for the team budget buttons
}
...
function showCeleb(name, price, team, color, logo, photo) {
  ...
  const isLast =
    teams.reduce((acc, t) => acc + t.squad.length, 0) === players.length;
...
function triggerAuctionComplete() {
  ...
  endScreen.innerHTML = `
    <div class="animate-bounce mb-4"><span class="bg-orange-600 text-white px-6 py-2 rounded-full font-black text-sm">Season 3 Finalized</span></div>
    <h1 class="text-6xl md:text-8xl font-black italic text-white mb-2 tracking-tighter uppercase">AUCTION COMPLETE</h1>
    ...
    <button onclick="location.reload()" class="bg-orange-600 text-white px-12 py-5 rounded-2xl font-black text-2xl shadow-2xl uppercase italic">View Final Squads</button>`;
...
document.addEventListener("keydown", (e) => {
  // If user is typing in the search box, don't trigger shortcuts
